def test_login_success(client, mock_redis_client):
...
def test_login_wrong_password(client, mock_redis_client):
    mock_redis_client.get.return_value = None
    response = client.post("/token", data={"username": "admin", "password": "wrong"})
    assert response.status_code == 401
    assert response.json()["detail"] == "Wrong credentials"

def test_brute_force_lockout(client, mock_redis_client):
    mock_redis_client.get.return_value = b"6"
    response = client.post("/token", data={"username": "admin", "password": "wrong"})
